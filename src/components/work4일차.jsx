//1. 검색입력으로 search 바꾸고
//2. todos + search + statusFilter 조합해서 보이는 모곡 개신
//3. 추가 폼으로 새 할일을 부모 콜백 호출로 등록
//Memoization 어떤 결과를 기억해 두었다가, 입력이 같으면 다시 계산하지 않고 기존 결과를 재사용 하는 것
export default function work4일차(){
  ...
  const addTodo=(title)=>{
    ...
    if(!trimed) return;
    const newItem= {id:Date.now(), title: trimed, done:false}
